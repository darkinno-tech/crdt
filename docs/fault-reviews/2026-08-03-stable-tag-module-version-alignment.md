...
```
./durable/go.mod requires github.com/darkinno-tech/crdt v1.0.35; expected v1.0.36 for v1.0.36
```
...
3. `scripts/tag-go-modules.sh` 扫描每个 `go.mod` 中的 `github.com/darkinno-tech/crdt` 依赖，并要求其等于目标标签版本。
