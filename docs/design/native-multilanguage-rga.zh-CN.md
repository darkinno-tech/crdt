...
Python、Rust、Swift、C++ 中各复制一份尚未验证的合并逻辑。仓库新增完整的 Rust
`rga-run-v2` 原生核心，并通过显式所有权的 C ABI 提供 Python、Swift 与 C++20
绑定。这些绑定已经支持本地编辑、合并、快照与恢复，但应准确称为“共享 Rust
语义核心的原生 runtime 绑定”，而不是独立语言的 wire 实现。
...
| 维度 | 独立四语言实现 | Rust 核心 + FFI | 本轮选择 |
| --- | --- | --- | --- |
| 正确性 | 四套 canonical 编码、HLC、乱序父节点和 tombstone 需同步修复 | 一套经过向量验证的解码、合并与 HLC 语义 | Rust 核心 + FFI |
| 安全性 | 四个不可信输入解码器与资源控制面 | 一个有界解码器；ABI 只转移拥有的 buffer | Rust 核心 + FFI |
| 性能 | 可能获得各语言最优实现，但前期无基准即有四倍优化面 | 原生 runtime；FFI 成本远小于一次 frame 合并 | Rust 核心 + FFI |
| 维护成本 | 四个发布、测试和兼容矩阵 | 一个 protocol core，语言层仅资源所有权/API | Rust 核心 + FFI |
...
可变内存借给 Python/Swift/C++，也不会让 panic 越过 FFI 边界。宿主仍负责 handle
生命周期和动态库签名/分发。
...
- `make cpp-test`：真实 C++20 → C ABI → Rust runtime 的 Go 向量、畸形帧原子拒绝、
  重复/乱序收敛与快照恢复链路。
- `make rust-benchmark`：受控本地 insert、frame relay、state/recovery 基准；仅是
  回归信号而不是生产容量承诺。
- `make cpp-benchmark`：受控 C++ facade → C ABI → native runtime 的对应基准；不代表
  C++ allocator、动态库分发或终端设备 SLA。
