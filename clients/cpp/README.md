# DarkInno C++ CRDT client

`darkinno::crdt::Rga` and `darkinno::crdt::LwwMap` are C++20 RAII facades over
...
containers, and throw `darkinno::crdt::Error` for a non-zero ABI status.
...
darkinno::crdt::LwwMap metadata("cpp-device-7");
