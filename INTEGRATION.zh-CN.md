...
1. 为每个存活逻辑副本分配全局唯一且非空的 ID。OR-Set 重启后若复用 ID，必须
   恢复 HLC 状态；否则要使用新 ID。
2. 本地变更时调用 `Add`、`Remove` 或 `Increment`，在同一个持久化事务/outbox 中
...
6. 交换实验性 LWW-Map、RGA 或 OR-Tree 帧前，必须对由
...
## 4. 实验性 LWW-Map、RGA 与 OR-Tree 集成

LWW-Map（`lww.Map`）、RGA（`text`）和 OR-Tree（`tree`）是带帧、带 HLC 的实验性
...
| 检查项 | 必需证据 |
| --- | --- |
| 稳定身份 | 已记录并测试 replica-ID 生命周期及 OR-Set HLC 持久化。 |
...
| 实验协议一致性 | 只有经过认证的双方 `ProtocolPolicy.FrameTypes()` 比对一致后才启用 LWW-Map/RGA/OR-Tree；其 HLC 状态已持久化且墓碑被保留。 |
