...
// TestRGAPackedV3ThreeReplicaEditingOverUnreliableNetwork repeats the editor
// path through the separately negotiated packed-v3 wire format. A clean
// receiver accepts the selection deletion before its referenced seed nodes,
// then receives duplicated frames in a different order before snapshot
// recovery. Tombstones must remain structural anchors for the concurrent edits.
func TestRGAPackedV3ThreeReplicaEditingOverUnreliableNetwork(t *testing.T) {
	alice := mustRGA(t, "packed-alice")
	bob := mustRGA(t, "packed-bob")
	carol := mustRGA(t, "packed-carol")
	dana := mustRGA(t, "packed-dana")

	base := mustInsertRGA(t, alice, 0, "Draft")
	for _, replica := range []*RGA{bob, carol} {
		mustApplyRGAPackedDelta(t, replica, base)
	}
	bobEdit := mustInsertRGA(t, bob, 5, " for review")
	carolEdit := mustInsertRGA(t, carol, 5, " collaboratively")

	// Alice merges both offline edits before deleting original seed runes.
	mustApplyRGAPackedDelta(t, alice, carolEdit)
	mustApplyRGAPackedDelta(t, alice, bobEdit)
	deleteEdit, err := alice.Delete(1, 2)
	if err != nil {
		t.Fatal(err)
	}
	...
	for index, replica := range []*RGA{alice, bob, carol, dana} {
		// Exercise delete-before-insert explicitly; later randomized duplicate
		// deliveries also verify idempotence and unresolved-parent replay.
		mustApplyRGAPackedDelta(t, replica, deleteEdit)
		deliverRGAPackedChanges(t, replica, changes, int64(20260801+index))
	}

	want := alice.String()
	for _, replica := range []*RGA{bob, carol, dana} {
		if got := replica.String(); got != want {
			t.Fatalf("replica text = %q, want %q", got, want)
		}
		if replica.PendingCount() != 0 {
	...
	saved, err := bob.SnapshotPackedCurrentState()
	if err != nil {
		t.Fatal(err)
	}
	recovered, err := NewFromSnapshot(saved)
...
func deliverRGAPackedChanges(t testing.TB, target *RGA, changes []Delta, seed int64) {
	t.Helper()
	frames := make([][]byte, 0, len(changes)*2)
	for _, change := range changes {
		encoded, err := change.MarshalPackedBinary()
		if err != nil {
			t.Fatal(err)
		}
		frames = append(frames, encoded, encoded)
	...
	random.Shuffle(len(frames), func(left, right int) { frames[left], frames[right] = frames[right], frames[left] })
	for _, encoded := range frames {
		change, err := UnmarshalRGAPackedDelta(encoded)
		if err != nil {
			t.Fatal(err)
		}
		if err := target.ApplyDelta(change); err != nil {
...
func mustApplyRGAPackedDelta(t testing.TB, value *RGA, change Delta) {
	t.Helper()
	encoded, err := change.MarshalPackedBinary()
	if err != nil {
		t.Fatal(err)
