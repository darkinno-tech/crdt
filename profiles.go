...
var replicationProfiles = [...]ReplicationProfile{
	...
	{
		ID:               "document/tree-v2",
		Title:            "Fully nested document tree",
		Summary:          "A bounded, fully replicated nested CRDT document protocol.",
		ConflictRule:     "Declared child CRDT operations merge within a versioned document-tree contract.",
		RecommendedFor:   []string{"structured collaborative documents whose complete hierarchy shares one replication, retention, and authorization boundary"},
		NotFor:           []string{"arbitrary recursive JSON", "mixed unnegotiated type IDs", "independently authorized or lazily loaded descendants", "dynamic plugin documents"},
		HostRequirements: []string{"Preflight identity, ownership, depth, width, bytes, operations, and pending work before mutation.", "Authorize and quota the entire tree as one group; partition content into distinct groups before it needs a different boundary.", "Persist roots, declarations, frontier, HLC state, and local counter atomically."},
