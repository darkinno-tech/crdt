...
func marshalHello(manifest replica.Manifest) ([]byte, error) {
	return json.Marshal(helloMessage{Version: protocolVersion, Manifest: manifest})
}
...
func unmarshalChange(data []byte, maxMessageBytes, maxActorBytes int) (replica.Dot, []byte, error) {
	if len(data) == 0 || len(data) > maxMessageBytes || maxActorBytes <= 0 || data[0] != protocolVersion {
