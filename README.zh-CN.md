...
已发布的根模块 `github.com/darkinno-tech/crdt` 没有任何非标准库 module 依赖。耐久存储、传输和数据库后端都是独立版本的按需模块，因此仅使用核心的应用不会解析它们的依赖图。

| 模块 | 按需能力 |
| --- | --- |
| `github.com/darkinno-tech/crdt/durable` | bbolt durable relay 与 WebSocket 重连客户端。 |
| `github.com/darkinno-tech/crdt/persistence` | bbolt 与文件 checkpoint Store。 |
| `github.com/darkinno-tech/crdt/telemetry` | 有界 telemetry 与按需 OpenTelemetry metrics adapter。 |
| `github.com/darkinno-tech/crdt/extensions` | WebSocket、HTTP/SSE、gRPC 与 Yjs relay 参考。 |
| `github.com/darkinno-tech/crdt/providers/{redis,postgres,mysql,mssql,sqlite,webrtc}` | durable-log 与 DataChannel 后端。 |
| `github.com/darkinno-tech/crdt/examples` | 可运行示例，包括 WebSocket 参考。 |
...
| `github.com/darkinno-tech/crdt/persistence` | 按需的本地有界 bbolt 和文件 CRDT checkpoint Store 参考。 |
| `github.com/darkinno-tech/crdt/telemetry` | 按需的无 payload 有界 telemetry 和 OpenTelemetry adapter。 |
| `github.com/darkinno-tech/crdt/durable`、`github.com/darkinno-tech/crdt/extensions`、`observe` | 按需的 durable relay 和 live relay；核心中的进程内观察。 |
